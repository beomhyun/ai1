...
def expected_sales(tv, rd, newspaper, beta_0, beta_1, beta_2, beta_3):
    ...
    예상되는 Sales 값을 출력합니다.
    """
    rs = ((tv * beta_0) + (rd * beta_1) + (newspaper * beta_2) + beta_3)
    return rs
...
파일명: model.clf
"""
with open("model.clf", "wb") as fw:
    pickle.dump(lrmodel, fw)
# joblib.dump(lrmodel, 'model.clf')
...
def N_LinearRegression(X, Y, iters, learning_rate):
    """
...
def prediction(beta_x, beta_3, X):
    # 예측 값을 계산하는 식을 만든다.
    # print(beta_x)
    # print(X)
    equation = [0]*len(X)
    ...
    for i in range(len(X)):
        tmp = np.dot(beta_x, X[i])
        # print(tmp)
        equation[i] = tmp + beta_3[0]

        # equation = beta_x[0] * X[i][0] + beta_x[1] * X[i][1] + beta_x[2] * X[i][2] + beta_3[0]
    # print(equation)
    return equation
...
def gradient_beta(X, error, lr):
    ...
    beta_x_delta = sum(X * (error**2))*lr / len(X)
    # beta_3를 업데이트하는 규칙을 정의한다.
    beta_3_delta = sum(error**2)*lr / len(error)
...
# X_test_pred에 테스트 데이터에 대한 예상 판매량을 모두 구하여 len(y_test) X 1 의 크기를 갖는 열벡터에 저장합니다.
N_X_test_pred = prediction(N_beta_x,N_beta_3,X_test)
